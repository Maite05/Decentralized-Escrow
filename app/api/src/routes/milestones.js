...
// POST /approve — deliver or approve a milestone
router.post('/approve', async (req, res, next) => {
  try {
    ...
    if (project.status === 'DISPUTED') {
      return res
        .status(409)
        .json({ error: 'Project is under dispute — approvals paused' });
    }
    ...
    if (!isClient && !isFreelancer) {
      return res
        .status(403)
        .json({ error: 'Wallet is not a party to this project' });
    }
    ...
    if (body.action === 'deliver' && isFreelancer) {
      if (milestone.status !== 'LOCKED') {
        return res.status(409).json({
          error: `Cannot deliver milestone in status: ${milestone.status}`,
        });
      }
      ...
      if (milestone.status !== 'DELIVERED') {
        return res.status(409).json({
          error: `Cannot approve milestone in status: ${milestone.status}`,
        });
      }
    ...
    // Auto-complete the project when all milestones are released
