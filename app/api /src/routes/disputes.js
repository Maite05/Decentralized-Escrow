...
const resolveDisputeSchema = z.object({
    ...
    resolution: z.string().min(10, "Resoltion must be at least 10 characters"),
    winner: z.enum(["client", "freelance", "split"]),
});
...
router.post("/"), async(req,res, next) => {
    try{
        ...
        const isParty =
      project.client.walletAddress === wallet || project.freelancer.walletAddress === wallet;
       
        if (!isParty) {
            return res.status(403).json({ error: "Only project parties can raise a dispute" });
    }
 
        if (project.dispute && project.dispute.status === "OPEN") {
            return res.status(409).json({ error: "An open dispute already exists for this project" });
    }

        if (project.status === "COMPLETED") {
            return res.status(409).json({ error: "Cannot dispute a completed project"});
    }
        ...
        const [dispute] = await prisma.$transaction([
          ...
          prisma.milestone.updateMany({
            where: { projectId: body.projectId, status: { in: ["LOCKED', 'DELIVERED"] } },
            data: { status: "DISPUTED" },
      }),
    ]);
...
router.get("/:projectId", async (req, res, next) => {
  try {
    ...
    if (!dispute) {
      return res.status(404).json({ error: "No dispute found for this project" });
    }
...
router.post("/:id/resolve", async (req, res, next) => {
  try {
    ...
    if (!mediator) {
      return res.status(403).json({ error: 'Wallet is not a registered mediator' });
    }
